...
{
    escribirVerde("Ingrese ID de la empresa a modificar\n");
    $id = trim(fgets(STDIN));
    $empresa = new Empresa();
...
function listarEmpresas()
{
    ...
    if ($empresas) {
        ...
        escribirRojo("Error al listar empresas: " . $empresa->getmensajeoperacion() . "\n");
    }
}

function eliminarEmpresa()
{
    escribirVerde("Ingrese ID de la empresa a eliminar\n");
    $id = trim(fgets(STDIN));
    $empresa = new Empresa();
    if ($empresa->Buscar($id)) {
        ...
        if ($empresa->eliminar()) {
            ...
            if (empty($empresa)) {
                // Si no hay empresas, reiniciamos el contador de ID
...
function listarViajes()
{
    ...
    if ($viajes) {
        ...
        escribirRojo("Error al listar viaje: " . $viaje->getmensajeoperacion() . "\n");
    }
}
...
function listarResponsables()
{
    ...
    if ($responsables) {
        ...
        escribirRojo("Error al listar responsable: " . $responsable->getmensajeoperacion() . "\n");
    }
}
...
function listarPasajeros($viaje) {
    ...
    if (count($colPasajeros) > 0) {
        ...
        escribirRojo("No hay pasajeros para este viaje.\n");
    }
}
...
function eliminarPasajero() {
    ...
    $listaPasajeros = $pasajero->listar(); // Suponiendo que listar() devuelve un array de objetos Pasajero
    $continuarEjecucion = true;
    // Verificar si hay pasajeros para eliminar
    if (empty($listaPasajeros)) {
        escribirRojo("No hay pasajeros para eliminar.\n");
